...
const getBrowserStyle = (level: LogLevel.LogLevel): string => {
  switch (level) {
    case LogLevel.Fatal: return 'background: #dc2626; color: white; font-weight: bold; padding: 2px 4px; border-radius: 3px'
    case LogLevel.Error: return 'color: #dc2626; font-weight: bold'
    case LogLevel.Warning: return 'color: #d97706; font-weight: bold'
    case LogLevel.Info: return 'color: #2563eb; font-weight: 500'
...
const getNodeColor = (level: LogLevel.LogLevel): string => {
  switch (level) {
    case LogLevel.Fatal: return '\x1b[41m\x1b[37m\x1b[1m'
    case LogLevel.Error: return '\x1b[91m\x1b[1m'
    case LogLevel.Warning: return '\x1b[93m\x1b[1m'
    case LogLevel.Info: return '\x1b[94m'
    case LogLevel.Debug: return '\x1b[37m'
    default: return ''
  }
}
...
const createBrowserFormatter = (level: LogLevel.LogLevel): LogFormatter =>
  (timestamp: string, message: string) => `%c${timestamp} ${message}`

const createNodeFormatter = (level: LogLevel.LogLevel): LogFormatter =>
...
const logToConsole = (level: LogLevel.LogLevel, formattedMessage: string, style?: string): void => {
  const methodName = getConsoleMethodName(level)
  
  switch (methodName) {
    case 'error':
      style ? console.error(formattedMessage, style) : console.error(formattedMessage)
      break
    case 'warn':
      style ? console.warn(formattedMessage, style) : console.warn(formattedMessage)
      break
    case 'info':
      style ? console.info(formattedMessage, style) : console.info(formattedMessage)
      break
    case 'debug':
      style ? console.debug(formattedMessage, style) : console.debug(formattedMessage)
      break
    default:
      style ? console.log(formattedMessage, style) : console.log(formattedMessage)
  }
}
...
const loggerLayer = EffectLogger.replace(EffectLogger.defaultLogger, effectLogger)
